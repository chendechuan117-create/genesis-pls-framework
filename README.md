...
这个模板是从完整 Genesis 仓库裁出的，所以仍保留一些辅助入口、历史入口或诊断脚本。它们存在，不等于 Yogg/PLS 最小路径依赖它们。

- **`discord_bot.py`**：完整 Genesis 的 Discord 入口；Yogg 放生模式不以它作为输入入口。
- **`genesis/mcp_server.py` / `start_api.sh`**：服务化或 MCP/API 接入入口；不是 PLS 点线面的核心路径。
- **`genesis/v4/trace_pipeline/`**：trace 后处理与查询能力，用于分析运行经验；不是 `record_point` / `record_line` / `SurfaceExpander` 的最小闭环。
- **`genesis/skills/`**：物理技能与工具扩展层；Yogg 可以加载工具，但 PLS 本身不依赖某个具体技能。
- **`scripts/`**：部署、巡检、仪表盘、迁移、实验脚本集合；其中不少服务于作者的运行环境，不应被当成新用户必须启动的后台系统。
- **`tests/`**：历史回归、实验验证和边界探针混在一起；它说明框架曾被测试过哪些行为，不等于所有测试都是新用户的必跑路径。

阅读这份 README 时，应优先看下面这条链路：

```text
yogg_auto.py -> run_auto() -> V4Loop -> search / record_point / record_line -> NodeVault + Surface
```

其他入口可以之后再研究，但不要把它们倒推成 Yogg 放生模式的必需架构。
